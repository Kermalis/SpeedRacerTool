...
internal sealed class XDSFile
{
	...
	public XDSFile(Stream s)
	{
		...
		uint fileType = r.ReadUInt32(); // Seems to indicate the type of file. All t01 physics ones are 0xAB90DE70 for example, between PS2 and WII
		...
		ushort unk24 = r.ReadUInt16(); // "Unk1" in the notes below

		AssertValue(fileLength, (ulong)(r.Stream.Length - r.Stream.Position));

		ushort unk26 = r.ReadUInt16();
		if (unk26 == 0)
		{
			throw new Exception();
		}
		...
		//  (ushort_LE) = ??? Unk1. Seems to affect how the following data is read, but not by much. So it might be an opcode or bitflags
		...
		//  Interestingly, PS2 tracks have a different filetype/Unk1 (0xE73FBE05 / 0x0131) than the WII tracks (0xF6EB4F8D / 0x012F)
		...
		//   Unk1 = 0x0106
		//  0x26 (ushort_LE) = amount of tracks. 6 in PS2, 5 in WII. This corresponds with the amount of nodes below.
		...
		//   Unk1 = 0x0106
		//  0x26 (ushort_LE) = 0x073C (1,852). There are 1,852 nodes below.
		...
		//  0x50 (LE)0x0009
		//  <
		//   0x52 [OneAyyArray](25 in PS2, 20 in WII) // each character entry is exactly 0x126 bytes
		//   {
	...
	private static void AssertValue(ulong value, ulong expected)
	{
		if (value != expected)
		{
			throw new Exception();
